...
%MyProgram% file8.txt > %OUT% || goto err
fc %OUT% "%~dp0file8-out.txt" > nul || goto err
echo test 8 has passed

%MyProgram% file9-no-path.txt > %OUT% || goto err
fc %OUT% "%~dp0file9-no-path-out.txt" > nul || goto err
