...
LittleAlgorithm::LittleAlgorithm(int vertexesNumber, vector<Edge> edges)
	: vertexesNumber(vertexesNumber),
	edges(std::move(edges)) {
	makeAdjacencyMatrix();
}

void LittleAlgorithm::makeAdjacencyMatrix() {
	data.assign(vertexesNumber + 1, std::vector<int>(vertexesNumber + 1, INT_MAX));
	for (const auto& e : edges) {
		data[e.edge.first - 1][e.edge.second - 1] = e.weight;
		data[e.edge.second - 1][e.edge.first - 1] = e.weight;
	}
}

void LittleAlgorithm::findHamiltonCycle()
{
	validateData();
